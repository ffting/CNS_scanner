...
An application is created that allows login by verifying credentials through a credentials.txt file created at the launch of the app. However, the app is vulnerable because it does not perform a version check to ensure that the app is not outdated with known vulnerabilities.

The app does not implement the update check mechanism as recommended by OWASP:
...
// Part 4: Checks that the update is not stalled during 'onResume()'.
